...
open-design unreviewed is 8138 of 12850 ledger rows. Reviewed rows are 1755. Batches 509–524 were length-checked and ledgered. Batches 525–1067 were not reviewed. Fixes include project-path confinement, escaped updater notes, http(s)-only browser open, OAuth script escaping, owner-only MCP files, hashed runtime cache key, JSON-LD escaping, launch-week keyframes, plugin slug `..` collapse, locale blog RSS aligned with blog locales, the English DeepSeek provider link, same-origin notification clicks, `no-store` on `/share-out/*`, and escaped community template preview text. `OD_DISABLE_API_AUTH`, deck thumbnail HTML, and cross-repo protocol changes were left. Waves 2 and 3 were not run.
