...
open-design unreviewed is 8378 of 12850 ledger rows. Reviewed rows are 1515. Batches through 508 that had landed were length-checked and ledgered, including 499, 500, and 503–508 after they returned. No further batches were started. Local unpushed fixes include project-path confinement, escaped updater notes, http(s)-only browser open, OAuth script escaping, owner-only MCP files, hashed runtime cache key, JSON-LD escaping, launch-week keyframes, plugin slug `..` collapse, locale blog RSS aligned with blog locales, and the English DeepSeek provider link. `OD_DISABLE_API_AUTH` and cross-repo protocol changes were left. Waves 2 and 3 were not run.
