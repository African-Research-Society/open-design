...
export function loadConfig(): AppConfig {
  try {
    ...
    const merged: AppConfig = {
      ...
      // Keep the workspace theme explicit. Legacy `system` values resolve to
      // light so CSS and embedded viewers never disagree about the active mode.
      theme: resolveAppTheme(parsed.theme),
      pet: normalizePet(parsed.pet),
      notifications: normalizeNotifications(parsed.notifications),
      orbit: normalizeOrbit(parsed.orbit),
    };
    // Rewrite only the retired system preference; explicit light and dark are
    // both supported workspace themes.
    let migratedConfig = parsed.theme === 'system';
